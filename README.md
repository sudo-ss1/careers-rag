...
The report page holds a **conversation**. Ask for a role, then narrow it in a
separate turn — `software engineer jobs` → `in Bangalore` → `remote only` — and
the session carries intent forward. Constraints accumulate; naming a new role
replaces the role and keeps the constraints. Every slot the session holds is
rendered as a chip you can remove, so the state is inspectable rather than hidden.

Two implementations of the same idea, for two different environments:

| | Web (`docs/`) | Repo (`chat.py`) |
|---|---|---|
| Turn fusion | deterministic slot-filling against a gazetteer built from the corpus | LLM query condensation against the conversation |
| Retrieval | BM25 from [`bm25.py`](src/careers_rag/bm25.py), ported to JS, ~0.3 ms | hybrid BM25 + embeddings, RRF |
| Answer | ranked postings | grounded prose with verified citations, or a refusal |
| Needs a key | no | yes |

A static page cannot hold an API key, and a hosted backend is one more thing that
can be down when someone opens the link — so the web path is engineered to work
offline and never hallucinate, and the LLM path lives in
[`chat.py`](src/careers_rag/chat.py) where a key is available:

```
you > software engineer jobs
you > in Bangalore
    [condensed: software engineer jobs in Bangalore]
```
